...
class FunCommands(commands.Cog):
    ...
    async def admin_help(self, ctx):
        ...
        embed.add_field(
            name="🎭 **Επικοινωνία, Roleplay & AI**",
            value=(
                "**`!say [#κανάλι] [κείμενο]`** - Το bot στέλνει το κείμενό σου στο κανάλι.\n"
                "**`!reply [LINK_ΜΗΝΥΜΑΤΟΣ] [κείμενο]`** - Το bot κάνει απευθείας reply στο μήνυμα.\n"
                "**`!on`** - Ενεργοποιεί το AI chatbot.\n"
                "**`!off`** - Απενεργοποιεί το AI (Sleep Mode).\n"
                "**`!reset`** - Καθαρίζει τη μνήμη συζητήσεων του AI (Memory Wipe)."
            ),
            inline=False
        )
